...
def gw_get(mid):
    return gw_all().get(str(mid))

def gw_put(rec):
    d = gw_all()
    d[str(rec["message_id"])] = rec
    gw_save_all(d)
    return rec
...
def gw_new_record(*, guild_id, channel_id, message_id, end_time, gagnants, gain,
                  emoji="🎉", organizer_id=None, conditions=None, data=None):
    return {
        "guild_id":     int(guild_id),
        "channel_id":   int(channel_id),
        "message_id":   int(message_id),
        "end_ts":       float(end_time.timestamp()),
        "gagnants":     max(1, int(gagnants or 1)),
        "gain":         gain,
        "emoji":        emoji or "🎉",
        "organizer_id": int(organizer_id) if organizer_id else None,
...
def gw_build_embed(rec, guild, participants=None):
    emoji = rec.get("emoji", "🎉")
    data  = rec.get("data", {}) or {}
    parts = [int(u) for u in (participants if participants is not None else rec.get("participants", []))]
    org   = rec.get("organizer_id")
    org_txt = f"<@{org}>" if org else (guild.owner.mention if guild and guild.owner else "Admin")
    e = discord.Embed(title=f"{emoji} GIVEAWAY {emoji}", color=C_BLUE)
    e.description = (
        f"🎁 **Gain :** {rec.get('gain', 'Un lot')}\n"
        f"👤 **Organisé par :** {org_txt}\n"
        f"🏆 **Gagnants :** {rec.get('gagnants', 1)}\n"
        f"👥 **Participants :** {len(parts)}\n"
        f"⏳ **Fin :** <t:{int(rec.get('end_ts', 0))}:R>\n\n"
        f"Réagis avec {emoji} pour participer !"
    )
    conds = rec.get("conditions") or []
    if conds and data.get("show_conditions", True):
        e.add_field(name="📋 Conditions", value="\n".join(conds), inline=False)
    if data.get("show_participants") and parts:
        val = ", ".join(f"<@{u}>" for u in parts[:20]) + ("..." if len(parts) > 20 else "")
        e.add_field(name="👥 Participants", value=val, inline=False)
    e.set_footer(text="ModeraBot • Giveaway")
    return e
...
async def gw_fetch_message(rec):
    guild = bot.get_guild(int(rec.get("guild_id") or 0))
    if not guild:
        return None, None, None
    cid = int(rec.get("channel_id") or 0)
...
async def gw_collect_participants(guild, msg, rec):
    ...
    blacklist = data.get("blacklist_roles") or []
    vocal     = data.get("vocal_required", False)
    org       = rec.get("organizer_id")
    out = []
    for u in users:
        if org and u.id == org:
            continue
        m = guild.get_member(u.id) if guild else None
        if required and (not m or not any(r.id in required for r in m.roles)):
            continue
        if blacklist and m and any(r.id in blacklist for r in m.roles):
            continue
        if vocal and (not m or not m.voice):
            continue
        out.append(u.id)
    return out
...
def gw_pick_winners(guild, rec, nb, pool=None, exclude=()):
    ...
    base    = [u for u in dict.fromkeys(base) if u not in exclude]
    if not base:
        return []
    weights = {u: 1 for u in base}
    for entry in str(data.get("bonus_raw", "") or "").split(","):
        p = entry.strip().split(":")
        if len(p) != 2:
            continue
        try:
            rid, bonus = int(p[0].strip()), int(p[1].strip())
        except:
            continue
        for u in base:
            m = guild.get_member(u) if guild else None
            if m and any(r.id == rid for r in m.roles):
                weights[u] = max(1, bonus)
    winners = []
    ...
    try:
        ...
        preset = None
    if preset and preset in base:
        winners.append(preset)
    remaining = [u for u in base if u not in winners]
    while remaining and len(winners) < max(1, int(nb or 1)):
        pick = random.choices(remaining, weights=[weights[u] for u in remaining], k=1)[0]
        winners.append(pick)
...
async def finish_giveaway(mid):
    ...
    parts   = await gw_collect_participants(guild, msg, rec)
    winners = gw_pick_winners(guild, rec, rec.get("gagnants", 1), pool=parts)
    ...
    if msg:
        try:
            done = discord.Embed(title="🎉 GIVEAWAY TERMINÉ !", color=C_GOLD)
            done.description = (
                f"🎁 **Gain :** {rec.get('gain', 'Un lot')}\n"
                f"👥 **Participants :** {len(parts)}\n"
                f"🏆 **Gagnant(s) :** {mentions or 'aucun'}"
            )
            done.set_footer(text=f"Reroll : +greroll {rec['message_id']}")
    ...
    if not winners:
        try:
            await channel.send("❌ Giveaway terminé : aucun participant valide.")
        except:
            pass
        return []
    ...
    end_e.add_field(name="🎁 Gain", value=str(rec.get("gain", "Un lot")), inline=False)
    if msg:
        end_e.add_field(name="🔗 Message", value=f"[Voir le giveaway]({msg.jump_url})", inline=False)
    end_e.set_footer(text=f"Reroll : +greroll {rec['message_id']}")
    try:
        await channel.send(embed=end_e)
        await channel.send(f"🎉 Félicitations {mentions} ! Tu as gagné **{rec.get('gain', '')}** !")
    except:
        pass
    return winners
...
async def gw_watcher():
    ...
    for mid, rec in list(gw_all().items()):
        if rec.get("ended"):
            continue
        try:
            if rec.get("end_ts", 0) <= now:
                await finish_giveaway(mid)
            elif now - _gw_last_refresh.get(mid, 0) >= 20:
                _gw_last_refresh[mid] = now
                guild, channel, msg = await gw_fetch_message(rec)
                if guild and msg:
                    parts = await gw_collect_participants(guild, msg, rec)
                    gw_patch(mid, participants=parts)
                    rec["participants"] = parts
                    await msg.edit(embed=gw_build_embed(rec, guild, parts))
        except Exception as ex:
            print(f"[giveaway] erreur sur {mid} : {ex}")

@gw_watcher.before_loop
async def _gw_watcher_before():
...
async def gw_engine_start():
    ...
    for mid in pending:
        try:
            ...
            print(f"[giveaway] erreur au démarrage sur {mid} : {ex}")
    if pending:
        print(f"🎉 {len(pending)} giveaway(s) en attente terminé(s) au démarrage")
...
async def greroll_cmd(ctx, message_id: int = None, nb_gagnants: int = None):
    ...
    if rec is None and not message_id:
        ...
        if not ended:
            return await ctx.send(embed=embed_err(
                "Usage : `+greroll <message_id> [nombre_gagnants]`\n"
    ...
    if rec is None:
        ...
        for r in target.reactions:
            ...
            break
        if not emoji:
            return await ctx.send(embed=embed_err("Aucune réaction sur ce message : impossible de retirer au sort."))
        rec = gw_new_record(
            guild_id=ctx.guild.id, channel_id=target.channel.id, message_id=target.id,
            end_time=target.created_at, gagnants=nb_gagnants or 1,
            gain="Giveaway", emoji=emoji, organizer_id=None,
        )
        rec["ended"] = True
        rec["ended_ts"] = discord.utils.utcnow().timestamp()
        gw_put(rec)
    ...
    if not rec.get("ended"):
        return await ctx.send(embed=embed_err(
            "Ce giveaway est toujours en cours — attends la fin avant de reroll."))

    guild, channel, msg = await gw_fetch_message(rec)
    guild = guild or ctx.guild
    parts = await gw_collect_participants(guild, msg, rec) if msg else [int(u) for u in rec.get("participants", [])]
    if not parts:
        return await ctx.send(embed=embed_err("Aucun participant valide trouvé sur ce giveaway."))

    nb   = max(1, nb_gagnants or rec.get("gagnants", 1))
    note = None
    already = {int(u) for u in rec.get("past_winners", [])}
    winners = gw_pick_winners(guild, rec, nb, pool=parts, exclude=already)
    if not winners:
        winners = gw_pick_winners(guild, rec, nb, pool=parts)
        note = "⚠️ Tous les participants avaient déjà gagné : re-tirage sur l'ensemble des participants."
    if not winners:
        return await ctx.send(embed=embed_err("Aucun participant valide trouvé sur ce giveaway."))

    rec["participants"] = parts
    rec["winners"]      = winners
    rec["past_winners"] = list(dict.fromkeys(list(already) + winners))
    gw_put(rec)

    mentions = ", ".join(f"<@{w}>" for w in winners)
    e = discord.Embed(title="🔁 REROLL GIVEAWAY", color=C_GOLD)
    e.add_field(name="🏆 Nouveau(x) gagnant(s)", value=mentions, inline=False)
    if rec.get("gain") and rec["gain"] != "Giveaway":
        e.add_field(name="🎁 Gain", value=str(rec["gain"]), inline=False)
    e.add_field(name="👥 Participants", value=str(len(parts)), inline=True)
    e.add_field(name="🚫 Anciens gagnants exclus", value=str(len(already)), inline=True)
    if msg:
        e.add_field(name="🔗 Message original", value=f"[Cliquez ici]({msg.jump_url})", inline=False)
    if note:
        e.add_field(name="ℹ️ Info", value=note, inline=False)
    e.set_footer(text=f"Reroll par {ctx.author} • {len(winners)} gagnant(s)")
...
async def gsend_cmd(ctx, salon: discord.TextChannel = None, duree: str = None, gagnants: int = None, *, gain: str = None):
    ...
    rec = gw_new_record(
        guild_id=ctx.guild.id, channel_id=salon.id, message_id=msg.id,
        end_time=end_time, gagnants=gagnants, gain=gain, emoji=emoji,
...
async def launch_giveaway_from_config(ctx_or_channel, data):
    ...
    rec = gw_new_record(
        ...
        end_time=end_time, gagnants=gagnants, gain=gain, emoji=emoji,
        organizer_id=organizer_id, conditions=conditions, data=data,
    )
    gw_put(rec)
    giveaways[msg.id] = rec
...
API_TOKEN = (os.environ.get("DASHBOARD_API_TOKEN")
              ...
              or "f92JkL0pA7xQ19Zb3T")

CLIENT_ID = (os.environ.get("DISCORD_CLIENT_ID")
              or CONFIG.get("client_id")
              or "1447165780708823140")

CLIENT_SECRET = (os.environ.get("DISCORD_CLIENT_SECRET")
              or CONFIG.get("client_secret")
              or "S1LZeqsNe0QqhwC9VJ7S-8_zfB9ITwZB")
...
# Un cookie "Secure" n'est jamais stocke sur une page en http:// simple :
...
# Le port doit correspondre a l'allocation du panel : SERVER_PORT est fourni
...
# ---------------------------------------------------------------
#  Envoi des panneaux depuis le dashboard
#
#  Le bouton « Envoyer le panneau » du dashboard appelle ces routes.
#  Elles reutilisent les vues du bot : un panneau publie ici se comporte
#  exactement comme celui envoye depuis +ticket ou +captcha.
# ---------------------------------------------------------------
...
def _panel_await(coro, timeout=25):
...
def _panel_embed_ticket(panel):
    try:
        color = int(str(panel.get("couleur") or "#5865F2").replace("#", ""), 16)
    except Exception:
        color = C_BLUE
    e = discord.Embed(
...
async def _panel_send_tickets(guild, channel):
    conf = (ts_load() or {}).get(str(guild.id)) or {}
    ...
    if mode == "bouton":
        ...
        view, embed = TicketButtonPanelView(guild, conf), _panel_embed_ticket(panel)
    elif mode == "container_v2":
        view = TicketContainerV2View(guild, conf)
        embed = discord.Embed(
            title=view._panel_title,
            description=view._panel_desc,
            color=view._panel_color,
        )
    else:
        view, embed = TicketSelectView2(guild, conf), _panel_embed_ticket(panel)
...
async def _panel_send_captcha(guild, channel):
    cfg = (_load_captcha() or {}).get(str(guild.id), {})
    if not cfg.get("enabled"):
    ...
    await channel.send(embed=e, view=CaptchaStartView(guild.id))
    return f"Panneau de verification publie dans #{channel.name}"


# Modules qui n'ont pas (encore) de panneau public cote bot : on renvoie la
# commande Discord a utiliser plutot qu'une erreur muette.
_PANEL_COMMANDES = {
    "giveaway":    "+giveaway",
    "apply":       "+apply",
...
_PANEL_ENVOIS = {
    "tickets": _panel_send_tickets,
    "captcha": _panel_send_captcha,
}


@app.route("/api/guild/<guild_id>/panel", methods=["POST"])
@app.route("/api/guild/<guild_id>/panel/<panel_key>", methods=["POST"])
@app.route("/api/guild/<guild_id>/send-panel", methods=["POST"])
def api_guild_panel(guild_id, panel_key=None):
    guild, member = _dash_auth(guild_id)
    if not guild:
        return jsonify({"error": "forbidden"}), 403

    body = request.get_json(silent=True) or {}
    key = str(panel_key or body.get("panel") or "").strip().lower()

    channel = guild.get_channel(_i(body.get("channel_id"), 0) or 0)
    ...
    envoi = _PANEL_ENVOIS.get(key)
    if envoi is None:
        commande = _PANEL_COMMANDES.get(key)
        message = (f"Ce panneau se publie sur Discord avec {commande}."
                   if commande else "Panneau inconnu.")
        return jsonify({"error": message}), 501

    try:
        detail = _panel_await(envoi(guild, channel))
    except PanelError as err:
        return jsonify({"error": str(err)}), 400
    except discord.Forbidden:
        return jsonify({"error": f"Discord refuse l'envoi dans #{channel.name}."}), 403
    except Exception as err:
        return jsonify({"error": f"Envoi impossible : {err}"}), 500
...
def run_api():
    ...
    try:
        ...
        print(f"🌐 Serveur web (waitress) sur 0.0.0.0:{WEB_PORT}")
...
# ══════════════════════════════════════════
# PACK MODULES SUPPLEMENTAIRES (modules_extra.py)
#   Le fichier doit etre depose A COTE de app.py.
#   S'il est absent, le bot demarre quand meme : seul le pack manque.
# ══════════════════════════════════════════

def _charger_modules_extra():
    ...
    chemin = os.path.join(dossier, "modules_extra.py")
    try:
        if os.path.exists(chemin):
            spec = importlib.util.spec_from_file_location("modules_extra", chemin)
            module = importlib.util.module_from_spec(spec)
            sys.modules["modules_extra"] = module
            spec.loader.exec_module(module)
        else:
            import modules_extra as module
    except ModuleNotFoundError:
        print("[modules_extra] fichier introuvable.")
        print(f"[modules_extra] depose modules_extra.py dans : {dossier}")
        return
    except Exception as err:
        print(f"[modules_extra] chargement impossible : {err}")
        return

    try:
        module.setup(bot, app)
    except Exception as err:
        print(f"[modules_extra] initialisation impossible : {err}")


_charger_modules_extra()
