...
# ─── Secrets ───────────────────────────────────────────────────────────────
# Rien en dur ici : ce fichier vit dans un dépôt public. Renseigne ces valeurs
# via les variables d'environnement, ou dans config.json (ignoré par git).
#   BOT_API_TOKEN, DISCORD_CLIENT_ID, DISCORD_CLIENT_SECRET,
#   OAUTH_REDIRECT_URI, FLASK_SECRET_KEY

API_TOKEN = os.environ.get("BOT_API_TOKEN") or CONFIG.get("api_token") or ""

CLIENT_ID = os.environ.get("DISCORD_CLIENT_ID") or CONFIG.get("client_id") or "1447165780708823140"

CLIENT_SECRET = os.environ.get("DISCORD_CLIENT_SECRET") or CONFIG.get("client_secret") or ""

REDIRECT_URI = (os.environ.get("OAUTH_REDIRECT_URI") or CONFIG.get("redirect_uri")
                or "http://5.178.107.228:2025/bot/auth/callback")

SECRET_KEY = os.environ.get("FLASK_SECRET_KEY") or CONFIG.get("flask_secret_key") or ""

if not SECRET_KEY:
    # Clé éphémère : les sessions Flask sont invalidées à chaque redémarrage.
    SECRET_KEY = os.urandom(32).hex()
    print("⚠️  FLASK_SECRET_KEY absent : clé de session générée au démarrage "
          "(les connexions seront perdues à chaque redémarrage).")

if not CLIENT_SECRET:
    print("⚠️  DISCORD_CLIENT_SECRET absent : /api/oauth-exchange ne pourra pas "
...
# ══════════════════════════════════════════
# CORS — le dashboard (dashboard.moderabot.xyz) est sur une AUTRE origine
# que cette API. Sans ces en-têtes le navigateur bloque toutes les requêtes
# et la page affiche « L'API du bot ne répond pas ».
#
# Origines autorisées : variable d'env DASHBOARD_ORIGINS (séparées par des
# virgules), sinon la liste par défaut ci-dessous.
# ══════════════════════════════════════════
...
def _cors_preflight(_unused):
    """Réponse au pré-vol CORS : les en-têtes sont ajoutés par _add_cors_headers."""
    return ("", 204)
...
def stats():
    ...
    if not API_TOKEN or request.args.get("key") != API_TOKEN: return jsonify({"error": "unauthorized"}), 403
