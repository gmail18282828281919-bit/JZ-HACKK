...
def is_premium(uid):
    ...
    """Premium actif ? (expires_at == 0 → premium a vie)"""
...
CMD_ALIASES = {
    ...
    "premiumpanel": ["premiumpanel","ppanel","panelpremium","premiumadmin","gestionpremium","pp","panelprem"],
...
async def antibot_cmd(ctx):
    ...
    # Fonctionnalite reservee au premium (fondateur, membre premium ou owner du serveur premium)

    if not premium_unlocked(ctx):

        e = discord.Embed(

            title="⭐ Anti-Bot — Fonctionnalité Premium",

            description=(

                "Le salon honeypot **Anti-Bot** est réservé aux membres **Premium**.\n\n"

                "🍯 Salon piège : 2 messages → kick, 3e → ban définitif\n"

                "📊 Historique complet des incidents\n\n"

                f"**Activer un code** : `+premium <code>`\n"

                f"**Obtenir un code** : 👉 [Serveur support]({PREMIUM_LINK})"
...
AIDE_PAGES = {
    ...
    "premium": (
        ...
        ("+premium", "Voir ton statut / activer un code"),

        ("+premium <code>", "Activer directement un code premium"),

        ("+premiumpanel", "👑 Panel fondateur : codes, durées, retraits, logs"),

        ("― ✨ AVANTAGES PREMIUM ―", ""),

        ("+antibot", "🚫 Salon honeypot anti-bot (kick/ban auto)"),
    ...
    "antiraid": (
        ...
        ("+antibot", "⭐ Premium — Créer le salon honeypot anti-bot"),
...
# ══════════════════════════════════════════════════
# PREMIUM — SYSTEME FONDATEUR (v2)
#   +premium        → utilisateurs (statut / activation d'un code)
#   +premiumpanel   → fondateurs  (panel complet de gestion)
# ══════════════════════════════════════════════════

PREMIUM_PERMANENT   = 0                                    # expires_at == 0 → premium a vie
PREMIUM_CODE_CHARS  = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789"   # sans I/O/0/1 (lisibilite)
PREMIUM_CODE_PREFIX = "JZ"
PREMIUM_LOG_MAX     = 500
PREMIUM_PAGE_SIZE   = 10

_PREMIUM_UNITS = {
    "s": 1, "m": 60, "h": 3600, "d": 86400, "j": 86400,
    "w": 604800, "mo": 2592000, "y": 31536000, "a": 31536000,
}
_PREMIUM_INFINITE = ("perm", "perma", "permanent", "infini", "inf", "lifetime", "vie", "avie", "∞")


def is_founder(user_id):
    """Fondateur / owner du bot (OWNER_IDS)."""
...
def premium_save(data):
    jsave(FILES["premium"], data)


def premium_logs_load():
    logs = jload(FILES["premium_logs"])
    if not isinstance(logs, dict):
        logs = {}
    logs.setdefault("activations", [])
    logs.setdefault("events", [])
    return logs


def premium_log_event(action, actor_id, target_id=None, details=""):
    """Historique de toutes les actions du panel (generation, ajout, retrait, edit...)."""
    try:
        logs = premium_logs_load()
        logs["events"].append({
            "action": action,
        ...
        logs["events"] = logs["events"][-PREMIUM_LOG_MAX:]
...
def parse_premium_duration(raw):
    """'30' → 30j · '12h' · '2w' · '6mo' · '1y' · 'perm' → a vie. None si invalide."""
    if raw is None:
        return None
    txt = str(raw).strip().lower().replace(" ", "")
    if not txt:
        return None
    if txt in _PREMIUM_INFINITE:
        return PREMIUM_PERMANENT
    m = re.fullmatch(r"(\d+)(mo|[smhdjwya])?", txt)
    if not m:
        return None
    value = int(m.group(1))
    if value <= 0:
        return None
    return value * _PREMIUM_UNITS[m.group(2) or "d"]


def fmt_premium_duration(seconds):
    if seconds == PREMIUM_PERMANENT:
        return "♾️ à vie"
    seconds = int(seconds)
    parts = []
    for unit, label in ((31536000, "an"), (2592000, "mois"), (86400, "j"), (3600, "h"), (60, "min")):
        if seconds >= unit:
            qty, seconds = divmod(seconds, unit)
            if label == "an":
                parts.append(f"{qty} an{'s' if qty > 1 else ''}")
            elif label == "mois":
                parts.append(f"{qty} mois")
            else:
...
def premium_expiry_text(expires_at):
    if not expires_at:
        return "♾️ **À vie**"
    return f"<t:{int(expires_at)}:R> · <t:{int(expires_at)}:d>"


# ─── Lecture / ecriture des acces ────────────────────────────────────────────

def premium_info(uid):
    """Infos premium actives d'un membre, sinon None."""
    data = premium_load()
...
def premium_active_users(data=None):
    """{uid: info} des premiums encore valides (a vie inclus)."""
    data = data or premium_load()
    now = int(time.time())
    return {uid: info for uid, info in data.get("users", {}).items()
            if not info.get("expires_at", 0) or info["expires_at"] > now}


def premium_grant(uid, seconds, actor_id, source="manuel", code=None, stack=True):
    """Ajoute / prolonge le premium. Retourne le nouvel expires_at (0 = a vie)."""
    data = premium_load()
    uid = str(uid)
    now = int(time.time())
    info = data["users"].get(uid, {})
    current = info.get("expires_at", 0)
    already_lifetime = bool(info) and current == PREMIUM_PERMANENT

    if seconds == PREMIUM_PERMANENT or already_lifetime:
        expires_at = PREMIUM_PERMANENT
    else:
        base = current if (stack and current > now) else now
        expires_at = base + int(seconds)

    history = list(info.get("history", []))
    history.append({"at": now, "by": str(actor_id), "source": source, "seconds": int(seconds)})
...
def premium_set_expiry(uid, expires_at, actor_id):
    """Force une date d'expiration precise (0 = a vie). False si le membre n'est pas premium."""
    data = premium_load()
    uid = str(uid)
    if uid not in data["users"]:
        return False
    data["users"][uid]["expires_at"] = int(expires_at)
    data["users"][uid]["warned"] = False
    history = list(data["users"][uid].get("history", []))
    history.append({"at": int(time.time()), "by": str(actor_id), "source": "edit", "expires_at": int(expires_at)})
    data["users"][uid]["history"] = history[-20:]
    premium_save(data)
    return True


def premium_revoke(uid):
    data = premium_load()
    uid = str(uid)
...
def premium_unlocked(ctx):
    """Fonctionnalite premium accessible : fondateur, membre premium, ou serveur dont l'owner est premium."""
    if is_founder(ctx.author.id):
        return True
    if is_premium(ctx.author.id):
        return True
    owner_id = getattr(ctx.guild, "owner_id", None)
    return bool(owner_id and is_premium(owner_id))


async def premium_sync_role(uid, add=True, reason="Premium"):
    """Ajoute / retire le role premium sur le serveur principal."""
    try:
...
async def premium_send_log(title, fields, color=None):
    try:
        ch = bot.get_channel(LOG_CHANNEL_ID)
        if not ch:
            return
        e = discord.Embed(title=title, color=color or C_GOLD, timestamp=datetime.now())
...
def premium_new_code():
    block = lambda n: "".join(random.choices(PREMIUM_CODE_CHARS, k=n))
    return f"{PREMIUM_CODE_PREFIX}-{block(4)}-{block(4)}-{block(4)}"


def premium_code_state(entry):
    """(disponible: bool, libelle: str)"""
    uses = entry.get("uses", 0)
    uses_max = entry.get("uses_max", 1)
    if uses_max <= 0:
        return True, f"♾️ illimité ({uses} util.)"
    if uses >= uses_max or entry.get("used"):
        return False, f"❌ épuisé ({uses}/{uses_max})"
    return True, f"✅ {uses}/{uses_max}"


def premium_create_codes(seconds, qty, actor_id, custom=None, uses_max=1, note=""):
    data = premium_load()
    created = []
    custom = (custom or "").strip().upper()
    for _ in range(qty):
        code = custom if (custom and qty == 1) else premium_new_code()
...
def premium_purge(codes_used=True, expired_users=True):
    """Nettoie les codes epuises et les acces expires. Retourne (codes, membres)."""
    data = premium_load()
    now = int(time.time())
    removed_codes = 0
    removed_users = 0
    if codes_used:
        for code, entry in list(data.get("codes", {}).items()):
            available, _ = premium_code_state(entry)
            if not available:
                del data["codes"][code]
                removed_codes += 1
    if expired_users:
        for uid, info in list(data.get("users", {}).items()):
            exp = info.get("expires_at", 0)
            if exp and exp <= now:
                del data["users"][uid]
                removed_users += 1
    premium_save(data)
    return removed_codes, removed_users


async def premium_redeem(code, user, guild=None):
    """Active un code pour un membre. Retourne (ok: bool, embed)."""
    data = premium_load()
    uid = str(user.id)
    ...
    if not entry:
        return False, discord.Embed(
            title="❌ Code invalide",
            description=f"Ce code n'existe pas ou a été supprimé.\n👉 [Obtenir un code]({PREMIUM_LINK})",
            color=C_RED)
    ...
    if not available:
        return False, discord.Embed(
            title="❌ Code épuisé",
            description=f"Ce code a déjà été utilisé.\n👉 [Serveur support]({PREMIUM_LINK})",
            color=C_RED)

    if uid in entry.get("used_by", []):
        return False, discord.Embed(
            title="❌ Déjà utilisé",
            description="Tu as déjà activé ce code.", color=C_RED)

    duration = entry.get("duration", 0)
    entry["uses"] = entry.get("uses", 0) + 1
    entry.setdefault("used_by", []).append(uid)
    uses_max = entry.get("uses_max", 1)
    entry["used"] = uses_max > 0 and entry["uses"] >= uses_max
    data["codes"][code] = entry
    premium_save(data)

    expires_at = premium_grant(uid, duration, actor_id=entry.get("created_by", "system"),
                               source="code", code=code)

    logs = premium_logs_load()
    logs["activations"].append({
        "user_id": uid, "code": code, "activated_at": now,
        "duration_seconds": duration, "expires_at": expires_at,
    })
    logs["activations"] = logs["activations"][-PREMIUM_LOG_MAX:]
    jsave(FILES["premium_logs"], logs)
    premium_log_event("activation", uid, uid, f"code `{code}` · {fmt_premium_duration(duration)}")

    await premium_sync_role(uid, add=True, reason=f"Activation premium ({code})")
    await premium_send_log("⭐ Activation Premium", [
        ("👤 Utilisateur", f"<@{uid}> (`{uid}`)", False),
        ("🔑 Code", f"`{code}`", True),
        ("⏳ Durée", fmt_premium_duration(duration), True),
        ("🏁 Expire", premium_expiry_text(expires_at), False),
    ])

    e = discord.Embed(
        title="⭐ Premium activé !",
        description=(
            f"Merci **{user.display_name}** ! Ton accès premium est actif.\n\n"
            f"🔑 Code : `{code}`\n"
            f"⏳ Durée ajoutée : **{fmt_premium_duration(duration)}**\n"
            f"🏁 Expiration : {premium_expiry_text(expires_at)}\n\n"
            "✨ Toutes les fonctionnalités premium sont débloquées (dont `+antibot`)."
        ),
        color=C_GOLD)
    e.set_footer(text="ModeraBot • Premium")
    return True, e
...
def build_premium_panel_embed(guild=None):
    data = premium_load()
    now = int(time.time())
    users = premium_active_users(data)
    codes = data.get("codes", {})

    lifetime = sum(1 for i in users.values() if not i.get("expires_at"))
    soon = sum(1 for i in users.values() if i.get("expires_at") and i["expires_at"] - now < 7 * 86400)
    available = sum(1 for c in codes.values() if premium_code_state(c)[0])
    used = len(codes) - available
    total_activations = len(premium_logs_load().get("activations", []))

    e = discord.Embed(
        title="👑 Panel Premium — Fondateurs",
        description=(
            "Gestion complète des accès premium : génération de codes, ajout manuel, "
            "modification de la durée, retrait et historique.\n"
            "**Utilise les boutons ci-dessous.**"
        ),
        color=C_GOLD, timestamp=datetime.now())
    e.add_field(name="👥 Membres actifs", value=f"**{len(users)}**", inline=True)
    e.add_field(name="♾️ À vie", value=f"**{lifetime}**", inline=True)
    e.add_field(name="⏳ Expirent < 7j", value=f"**{soon}**", inline=True)
    e.add_field(name="🔑 Codes dispo", value=f"**{available}**", inline=True)
    e.add_field(name="✅ Codes épuisés", value=f"**{used}**", inline=True)
    e.add_field(name="📊 Activations", value=f"**{total_activations}**", inline=True)

    if users:
        ordered = sorted(users.items(), key=lambda kv: (kv[1].get("expires_at") or float("inf")))
        lines = [f"`{i+1}.` <@{uid}> → {premium_expiry_text(info.get('expires_at', 0))}"
                 for i, (uid, info) in enumerate(ordered[:6])]
        if len(ordered) > 6:
            lines.append(f"*… et {len(ordered) - 6} autre(s) — bouton 👥 Membres*")
        e.add_field(name="📋 Prochaines expirations", value="\n".join(lines), inline=False)
    else:
        e.add_field(name="📋 Membres", value="*Aucun membre premium pour le moment.*", inline=False)

    if guild:
        role = guild.get_role(PREMIUM_ROLE_ID)
        e.add_field(name="🎭 Rôle premium",
                    value=role.mention if role else f"`{PREMIUM_ROLE_ID}` (introuvable ici)",
                    inline=False)
    e.set_footer(text="ModeraBot • Panel fondateur")
    return e


def build_premium_user_embed(user):
    info = premium_info(user.id)
    if is_founder(user.id):
        e = discord.Embed(
            title="👑 Fondateur",
            description=("Tu as un accès **illimité** à toutes les fonctionnalités.\n"
                         "🛠️ Ouvre le panel de gestion avec **`+premiumpanel`**."),
            color=C_GOLD)
        e.set_thumbnail(url=user.display_avatar.url)
        return e
    if info:
        e = discord.Embed(title="⭐ Ton Premium", color=C_GOLD)
        e.description = (
            f"✅ Statut : **Actif**\n"
            f"🏁 Expiration : {premium_expiry_text(info.get('expires_at', 0))}\n"
            f"🔑 Source : `{info.get('source', 'manuel')}`"
        )
        e.add_field(
            name="✨ Tes avantages",
            value=("🚫 `+antibot` — salon honeypot anti-bot\n"
                   "🔐 `+captcha` — vérification des membres\n"
                   "🎫 `+ticket` — menu déroulant\n"
                   "💾 `+backup` — jusqu'à 20 sauvegardes\n"
                   "📩 `+joinmp` / 👋 `+depart` — messages personnalisés"),
            inline=False)
        e.set_thumbnail(url=user.display_avatar.url)
        e.set_footer(text="ModeraBot • Premium")
        return e
    ...
    e.description = (
        "❌ Tu n'as **pas** de premium actif.\n\n"
        "**Activer un code**\n"
        "> `+premium <code>` ou le bouton ci-dessous\n\n"
        "**Obtenir un code**\n"
        f"> Rejoins le support et ouvre un ticket 👉 [Rejoindre]({PREMIUM_LINK})"
    )
    e.add_field(
        name="✨ Ce que débloque le premium",
        value=("🚫 `+antibot` — salon honeypot anti-bot\n"
               "🔐 `+captcha` — vérification des membres\n"
               "🎫 `+ticket` — menu déroulant\n"
               "💾 `+backup` — jusqu'à 20 sauvegardes\n"
...
class PremiumGenCodeModal(discord.ui.Modal, title="🔑 Générer des codes premium"):
    t_duration = discord.ui.TextInput(label="Durée (30 = 30j · 12h · 2w · 6mo · perm)",
                                      placeholder="30", max_length=10)
    t_qty      = discord.ui.TextInput(label="Quantité de codes (1-20)", placeholder="1",
                                      max_length=2, required=False, default="1")
    t_uses     = discord.ui.TextInput(label="Utilisations par code (0 = illimité)", placeholder="1",
                                      max_length=3, required=False, default="1")
    t_code     = discord.ui.TextInput(label="Code personnalisé (si quantité = 1)",
                                      placeholder="Laisse vide pour un code auto", required=False, max_length=32)
    t_note     = discord.ui.TextInput(label="Note interne (optionnel)",
                                      placeholder="Ex: giveaway septembre", required=False, max_length=100)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        seconds = parse_premium_duration(self.t_duration.value)
        if seconds is None:
            return await interaction.response.send_message(
                embed=embed_err("Durée invalide. Exemples : `30`, `12h`, `2w`, `6mo`, `perm`."), ephemeral=True)
        try:
            qty = max(1, min(20, int((self.t_qty.value or "1").strip() or 1)))
            uses_max = max(0, min(999, int((self.t_uses.value or "1").strip() or 1)))
        except ValueError:
            return await interaction.response.send_message(
                embed=embed_err("Quantité / utilisations invalides."), ephemeral=True)

        codes = premium_create_codes(seconds, qty, interaction.user.id,
                                     custom=self.t_code.value, uses_max=uses_max,
                                     note=self.t_note.value or "")
        if not codes:
            return await interaction.response.send_message(
                embed=embed_err("Aucun code généré (ce code existe déjà ?)."), ephemeral=True)

        premium_log_event("gen_code", interaction.user.id, None,
                          f"{len(codes)} code(s) · {fmt_premium_duration(seconds)} · {uses_max or '∞'} util.")
        e = discord.Embed(
            title="✅ Codes générés",
            description="```\n" + "\n".join(codes) + "\n```",
            color=C_GOLD)
        e.add_field(name="⏳ Durée", value=fmt_premium_duration(seconds), inline=True)
        e.add_field(name="🔁 Utilisations", value="♾️ illimité" if uses_max == 0 else str(uses_max), inline=True)
        e.add_field(name="📦 Quantité", value=str(len(codes)), inline=True)
        if self.t_note.value:
            e.add_field(name="📝 Note", value=self.t_note.value, inline=False)
        e.set_footer(text="Les membres activent avec +premium <code>")
        await interaction.response.send_message(embed=e, ephemeral=True)
        await premium_refresh_panel(self.panel)


class PremiumDeleteCodeModal(discord.ui.Modal, title="🗑️ Supprimer un code"):
    t_code = discord.ui.TextInput(label="Code à supprimer", placeholder="JZ-XXXX-XXXX-XXXX", max_length=32)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        code = self.t_code.value.strip().upper()
        if not premium_delete_code(code):
            return await interaction.response.send_message(
                embed=embed_err(f"Code `{code}` introuvable."), ephemeral=True)
        premium_log_event("del_code", interaction.user.id, None, f"code `{code}`")
        await interaction.response.send_message(embed=embed_ok(f"Code `{code}` supprimé."), ephemeral=True)
        await premium_refresh_panel(self.panel)


class PremiumGrantModal(discord.ui.Modal, title="➕ Donner le premium"):
    t_uid      = discord.ui.TextInput(label="ID ou mention du membre", placeholder="123456789012345678", max_length=32)
    t_duration = discord.ui.TextInput(label="Durée (30 = 30j · 12h · 6mo · perm)", placeholder="30", max_length=10)
    t_reason   = discord.ui.TextInput(label="Raison (optionnel)", placeholder="Ex: gagnant giveaway",
    ...
    async def on_submit(self, interaction: discord.Interaction):
        uid = _premium_extract_id(self.t_uid.value)
        if not uid:
            return await interaction.response.send_message(embed=embed_err("ID invalide."), ephemeral=True)
        seconds = parse_premium_duration(self.t_duration.value)
        if seconds is None:
            return await interaction.response.send_message(
                embed=embed_err("Durée invalide. Exemples : `30`, `12h`, `6mo`, `perm`."), ephemeral=True)

        reason = self.t_reason.value or "Ajout manuel"
        expires_at = premium_grant(uid, seconds, interaction.user.id, source="fondateur")
        await premium_sync_role(uid, add=True, reason=f"Premium ajouté par {interaction.user}")
        premium_log_event("grant", interaction.user.id, uid,
                          f"{fmt_premium_duration(seconds)} · {reason}")
        await premium_send_log("➕ Premium accordé", [
            ("👤 Membre", f"<@{uid}> (`{uid}`)", False),
            ("👑 Par", interaction.user.mention, True),
            ("⏳ Durée", fmt_premium_duration(seconds), True),
            ("🏁 Expire", premium_expiry_text(expires_at), False),
            ("📝 Raison", reason, False),
        ])
        await interaction.response.send_message(embed=discord.Embed(
            description=(f"✅ Premium accordé à <@{uid}>.\n"
                         f"⏳ **{fmt_premium_duration(seconds)}** · 🏁 {premium_expiry_text(expires_at)}"),
            color=C_GREEN), ephemeral=True)
        await premium_dm(uid, discord.Embed(
            title="⭐ Premium activé",
            description=(f"Un fondateur vient de t'offrir le premium !\n"
                         f"🏁 Expiration : {premium_expiry_text(expires_at)}"),
            color=C_GOLD))
        await premium_refresh_panel(self.panel)


class PremiumEditTimeModal(discord.ui.Modal, title="⏱️ Modifier la durée"):
    t_uid = discord.ui.TextInput(label="ID ou mention du membre", placeholder="123456789012345678", max_length=32)
    t_op  = discord.ui.TextInput(label="Opération : +7d · -3d · =30d · perm",
                                 placeholder="+7d", max_length=12)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if uid not in data.get("users", {}):
            return await interaction.response.send_message(
                embed=embed_err(f"<@{uid}> n'a pas de premium. Utilise **➕ Ajouter** d'abord."), ephemeral=True)

        raw = self.t_op.value.strip().lower().replace(" ", "")
        op = raw[0] if raw[:1] in ("+", "-", "=") else "+"
        value = raw[1:] if raw[:1] in ("+", "-", "=") else raw
        seconds = parse_premium_duration(value)
        if seconds is None:
            return await interaction.response.send_message(
                embed=embed_err("Opération invalide. Exemples : `+7d`, `-3d`, `=30d`, `perm`."), ephemeral=True)

        now = int(time.time())
        current = data["users"][uid].get("expires_at", 0)

        if seconds == PREMIUM_PERMANENT:
            new_exp = PREMIUM_PERMANENT
        elif op == "=":
            new_exp = now + seconds
        elif op == "-":
            if current == PREMIUM_PERMANENT:
                return await interaction.response.send_message(
                    embed=embed_err("Ce membre est premium **à vie** : utilise `=30d` ou retire son premium."),
                    ephemeral=True)
            new_exp = max(now, current - seconds)
        else:
            if current == PREMIUM_PERMANENT:
                return await interaction.response.send_message(
                    embed=embed_warn("Ce membre est déjà premium **à vie**."), ephemeral=True)
            new_exp = max(now, current) + seconds

        premium_set_expiry(uid, new_exp, interaction.user.id)
        expired = bool(new_exp) and new_exp <= now
        await premium_sync_role(uid, add=not expired, reason=f"Durée premium modifiée par {interaction.user}")
        premium_log_event("edit_time", interaction.user.id, uid, f"{op}{value}")
        await premium_send_log("⏱️ Durée premium modifiée", [
            ("👤 Membre", f"<@{uid}> (`{uid}`)", False),
            ("👑 Par", interaction.user.mention, True),
            ("🔧 Opération", f"`{op}{value}`", True),
            ("🏁 Nouvelle expiration", premium_expiry_text(new_exp), False),
        ])
        await interaction.response.send_message(embed=discord.Embed(
            description=(f"✅ Durée mise à jour pour <@{uid}>.\n"
                         f"🏁 Nouvelle expiration : {premium_expiry_text(new_exp)}"),
            color=C_GREEN), ephemeral=True)
        await premium_refresh_panel(self.panel)


class PremiumRevokeModal(discord.ui.Modal, title="❌ Retirer le premium"):
    t_uid    = discord.ui.TextInput(label="ID ou mention du membre", placeholder="123456789012345678", max_length=32)
    t_reason = discord.ui.TextInput(label="Raison (optionnel)", required=False, max_length=100)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        uid = _premium_extract_id(self.t_uid.value)
        if not uid or not premium_revoke(uid):
            return await interaction.response.send_message(
                embed=embed_err("Ce membre n'a pas de premium."), ephemeral=True)
        reason = self.t_reason.value or "Retrait manuel"
        await premium_sync_role(uid, add=False, reason=f"Premium retiré par {interaction.user}")
        premium_log_event("revoke", interaction.user.id, uid, reason)
        await premium_send_log("❌ Premium retiré", [
            ("👤 Membre", f"<@{uid}> (`{uid}`)", False),
            ("👑 Par", interaction.user.mention, True),
            ("📝 Raison", reason, False),
        ], color=C_RED)
        await interaction.response.send_message(
            embed=embed_ok(f"Premium retiré pour <@{uid}>."), ephemeral=True)
        await premium_dm(uid, discord.Embed(
            title="⭐ Premium retiré",
            description=f"Ton accès premium a été retiré.\n📝 Raison : {reason}",
            color=C_RED))
        await premium_refresh_panel(self.panel)


class PremiumActivateModal(discord.ui.Modal, title="⭐ Activer un code premium"):
    t_code = discord.ui.TextInput(label="Ton code premium", placeholder="JZ-XXXX-XXXX-XXXX", max_length=32)

    async def on_submit(self, interaction: discord.Interaction):
        ok, embed = await premium_redeem(self.t_code.value, interaction.user, interaction.guild)
        await interaction.response.send_message(embed=embed, ephemeral=True)


# ─── Utilitaires panel ───────────────────────────────────────────────────────

def _premium_extract_id(raw):
    digits = re.sub(r"\D", "", str(raw or ""))
    return digits if 15 <= len(digits) <= 22 else None


async def premium_dm(uid, embed):
    try:
...
async def premium_refresh_panel(panel):
    """Rafraichit le message du panel apres une action."""
    if not panel or not getattr(panel, "message", None):
        return
    try:
        guild = panel.message.guild
        await panel.message.edit(embed=build_premium_panel_embed(guild), view=panel)
    except Exception:
        pass


def _premium_paginate(lines, title, color=C_GOLD, empty="*Rien à afficher.*"):
    if not lines:
        return [discord.Embed(title=title, description=empty, color=color)]
    pages = []
    chunks = [lines[i:i + PREMIUM_PAGE_SIZE] for i in range(0, len(lines), PREMIUM_PAGE_SIZE)]
    for idx, chunk in enumerate(chunks):
        e = discord.Embed(title=title, description="\n".join(chunk), color=color)
        e.set_footer(text=f"Page {idx + 1}/{len(chunks)} • {len(lines)} entrée(s)")
        pages.append(e)
    return pages


class PremiumPagesView(discord.ui.View):
    """Navigation simple pour les listes (codes, membres, logs)."""

    def __init__(self, pages, author_id, raw_text=None):
        ...
        if not raw_text:
            self.remove_item(self.export_btn)

    async def interaction_check(self, interaction: discord.Interaction):
        if interaction.user.id != self.author_id:
            await interaction.response.send_message(embed=embed_err("Ce menu n'est pas pour toi."), ephemeral=True)
            return False
        return True
    ...
    @discord.ui.button(label="📋 Copier", style=discord.ButtonStyle.primary)
    async def export_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        text = self.raw_text or ""
        if len(text) > 1900:
            await interaction.response.send_message(
                file=discord.File(io.BytesIO(text.encode()), filename="premium_codes.txt"), ephemeral=True)
        else:
            await interaction.response.send_message(f"```\n{text}\n```", ephemeral=True)


class PremiumPurgeConfirmView(discord.ui.View):
    def __init__(self, author_id, panel=None):
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        codes, users = premium_purge()
        premium_log_event("purge", interaction.user.id, None, f"{codes} code(s), {users} membre(s)")
        for item in self.children:
            item.disabled = True
        await interaction.response.edit_message(embed=embed_ok(
            f"Nettoyage terminé : **{codes}** code(s) épuisé(s) et **{users}** accès expiré(s) supprimés."),
            view=self)
        await premium_refresh_panel(self.panel)
    ...
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        for item in self.children:
            item.disabled = True
        await interaction.response.edit_message(embed=embed_warn("Nettoyage annulé."), view=self)
...
class PremiumPanelView(discord.ui.View):
    def __init__(self, founder_id):
        ...
        self.founder_id = int(founder_id)
        self.message = None

    async def interaction_check(self, interaction: discord.Interaction):
        if not is_founder(interaction.user.id):
            await interaction.response.send_message(
                embed=discord.Embed(description="👑 Panel réservé aux **fondateurs**.", color=C_RED), ephemeral=True)
            return False
        if interaction.user.id != self.founder_id:
            await interaction.response.send_message(
                embed=embed_warn("Ouvre ton propre panel avec `+premiumpanel`."), ephemeral=True)
            return False
        return True
    ...
    # ── Ligne 1 : codes ──
    @discord.ui.button(label="Générer codes", emoji="🔑", style=discord.ButtonStyle.success, row=0)
    async def btn_gen(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(PremiumGenCodeModal(panel=self))

    @discord.ui.button(label="Liste des codes", emoji="📜", style=discord.ButtonStyle.secondary, row=0)
    async def btn_codes(self, interaction: discord.Interaction, button: discord.ui.Button):
        codes = premium_load().get("codes", {})
        ordered = sorted(codes.items(), key=lambda kv: kv[1].get("created_at", 0), reverse=True)
        lines, raw = [], []
        for code, entry in ordered:
            available, state = premium_code_state(entry)
            note = f" · 📝 {entry['note']}" if entry.get("note") else ""
            lines.append(f"{'🟢' if available else '🔴'} `{code}` — "
                         f"**{fmt_premium_duration(entry.get('duration', 0))}** · {state}{note}")
            if available:
                raw.append(code)
        pages = _premium_paginate(lines, "📜 Codes premium", empty="*Aucun code. Utilise 🔑 Générer codes.*")
        view = PremiumPagesView(pages, interaction.user.id, raw_text="\n".join(raw) if raw else None)
        await interaction.response.send_message(embed=pages[0], view=view, ephemeral=True)

    @discord.ui.button(label="Supprimer code", emoji="🗑️", style=discord.ButtonStyle.danger, row=0)
    async def btn_del_code(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(PremiumDeleteCodeModal(panel=self))

    # ── Ligne 2 : membres ──
    @discord.ui.button(label="Ajouter premium", emoji="➕", style=discord.ButtonStyle.success, row=1)
    async def btn_grant(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(PremiumGrantModal(panel=self))

    @discord.ui.button(label="Modifier durée", emoji="⏱️", style=discord.ButtonStyle.primary, row=1)
    async def btn_edit(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(PremiumEditTimeModal(panel=self))

    @discord.ui.button(label="Retirer premium", emoji="❌", style=discord.ButtonStyle.danger, row=1)
    async def btn_revoke(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(PremiumRevokeModal(panel=self))

    # ── Ligne 3 : consultation ──
    @discord.ui.button(label="Membres premium", emoji="👥", style=discord.ButtonStyle.secondary, row=2)
    async def btn_members(self, interaction: discord.Interaction, button: discord.ui.Button):
        users = premium_active_users()
        ordered = sorted(users.items(), key=lambda kv: (kv[1].get("expires_at") or float("inf")))
        lines = []
        for uid, info in ordered:
            src = info.get("source", "manuel")
            lines.append(f"<@{uid}> `{uid}`\n┗ {premium_expiry_text(info.get('expires_at', 0))} · `{src}`")
        pages = _premium_paginate(lines, "👥 Membres premium", empty="*Aucun membre premium actif.*")
        await interaction.response.send_message(
            embed=pages[0], view=PremiumPagesView(pages, interaction.user.id), ephemeral=True)

    @discord.ui.button(label="Historique", emoji="📊", style=discord.ButtonStyle.secondary, row=2)
    async def btn_logs(self, interaction: discord.Interaction, button: discord.ui.Button):
        logs = premium_logs_load()
        icons = {"activation": "⭐", "gen_code": "🔑", "del_code": "🗑️", "grant": "➕",
                 "edit_time": "⏱️", "revoke": "❌", "purge": "🧹", "expire": "⌛"}
        events = list(reversed(logs.get("events", [])))
        lines = []
        for ev in events:
            icon = icons.get(ev.get("action"), "•")
            target = f"<@{ev['target_id']}> " if ev.get("target_id") else ""
            lines.append(f"{icon} {target}{ev.get('details', '')} — <t:{ev.get('at', 0)}:R>\n"
                         f"┗ par <@{ev.get('actor_id')}>")
        pages = _premium_paginate(lines, "📊 Historique Premium", empty="*Aucune action enregistrée.*")
        await interaction.response.send_message(
            embed=pages[0], view=PremiumPagesView(pages, interaction.user.id), ephemeral=True)

    @discord.ui.button(label="Nettoyer", emoji="🧹", style=discord.ButtonStyle.secondary, row=2)
    async def btn_purge(self, interaction: discord.Interaction, button: discord.ui.Button):
        data = premium_load()
        now = int(time.time())
        codes = sum(1 for c in data.get("codes", {}).values() if not premium_code_state(c)[0])
        users = sum(1 for i in data.get("users", {}).values()
                    if i.get("expires_at") and i["expires_at"] <= now)
        e = discord.Embed(
            title="🧹 Nettoyage premium",
            description=(f"Supprimer **{codes}** code(s) épuisé(s) et **{users}** accès expiré(s) ?\n"
                         "*Les membres premium actifs ne sont pas touchés.*"),
            color=C_ORANGE)
        await interaction.response.send_message(
            embed=e, view=PremiumPurgeConfirmView(interaction.user.id, panel=self), ephemeral=True)

    @discord.ui.button(label="Actualiser", emoji="🔄", style=discord.ButtonStyle.primary, row=3)
    async def btn_refresh(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.edit_message(
            embed=build_premium_panel_embed(interaction.guild), view=self)

    @discord.ui.button(label="Fermer", emoji="🔒", style=discord.ButtonStyle.danger, row=3)
...
class PremiumUserView(discord.ui.View):
    """Vue affichee aux membres (activation d'un code)."""

    def __init__(self, author_id):
        super().__init__(timeout=300)
        self.author_id = int(author_id)
        if not is_founder(author_id):
            self.remove_item(self.btn_panel)
        self.add_item(discord.ui.Button(label="Obtenir le premium", emoji="🛒",
                                        style=discord.ButtonStyle.link, url=PREMIUM_LINK))
    ...
    @discord.ui.button(label="Activer un code", emoji="⭐", style=discord.ButtonStyle.success)
    ...
    @discord.ui.button(label="Panel fondateur", emoji="👑", style=discord.ButtonStyle.primary)
    async def btn_panel(self, interaction: discord.Interaction, button: discord.ui.Button):
        view = PremiumPanelView(interaction.user.id)
        await interaction.response.send_message(
            embed=build_premium_panel_embed(interaction.guild), view=view, ephemeral=True)
        try:
...
@bot.command(name="premiumpanel", aliases=["ppanel", "panelpremium", "premiumadmin", "gestionpremium", "pp"])
async def premiumpanel_cmd(ctx):
    """Panel de gestion premium — fondateurs uniquement."""
    if not is_founder(ctx.author.id):
        return await ctx.send(embed=discord.Embed(
            title="👑 Accès refusé",
            description="Le panel premium est réservé aux **fondateurs** du bot.",
            color=C_RED))
    view = PremiumPanelView(ctx.author.id)
    view.message = await ctx.send(embed=build_premium_panel_embed(ctx.guild), view=view)


@bot.command(name="premium", aliases=["prem", "prm", "abonnement"])
async def premium_cmd(ctx, *, arg: str = None):
    # Fondateur : "+premium @membre" ou "+premium <id>" → fiche du membre
    if arg and is_founder(ctx.author.id):
        target_id = _premium_extract_id(arg)
        if target_id:
            info = premium_info(target_id)
            e = discord.Embed(title="⭐ Fiche premium", color=C_GOLD if info else C_RED)
            e.description = f"👤 <@{target_id}> (`{target_id}`)"
            if info:
                e.add_field(name="✅ Statut", value="Actif", inline=True)
                e.add_field(name="🔑 Source", value=f"`{info.get('source', 'manuel')}`", inline=True)
                e.add_field(name="🏁 Expiration", value=premium_expiry_text(info.get("expires_at", 0)), inline=False)
                if info.get("granted_by"):
                    e.add_field(name="👑 Accordé par", value=f"<@{info['granted_by']}>", inline=True)
            else:
                e.add_field(name="❌ Statut", value="Aucun premium actif", inline=False)
            e.set_footer(text="Gestion complète : +premiumpanel")
            return await ctx.send(embed=e)

    # Aucun argument → statut personnel
    if not arg:
        return await ctx.send(embed=build_premium_user_embed(ctx.author),
                              view=PremiumUserView(ctx.author.id))

    # Sinon → activation du code
    ok, embed = await premium_redeem(arg.split()[0], ctx.author, ctx.guild)
    await ctx.send(embed=embed)


@tasks.loop(seconds=60)
async def check_premium_expirations():
    """Retire les premiums expires et previent les membres 24h avant."""
    try:
        data = premium_load()
        now = int(time.time())
        changed = False
        ...
        for uid, info in list(data.get("users", {}).items()):
            expires_at = info.get("expires_at", 0)
            if not expires_at:          # a vie
                continue

            if expires_at <= now:
                del data["users"][uid]
                changed = True
                await premium_sync_role(uid, add=False, reason="Premium expiré")
                premium_log_event("expire", "system", uid, "premium expiré")
                await premium_dm(uid, discord.Embed(
                    title="⌛ Premium expiré",
                    description=("Ton accès premium vient d'expirer.\n"
                                 f"👉 [Renouveler]({PREMIUM_LINK})"),
                    color=C_ORANGE))
                await premium_send_log("⌛ Premium expiré", [
                    ("👤 Membre", f"<@{uid}> (`{uid}`)", False),
                ], color=C_ORANGE)

            elif expires_at - now <= 86400 and not info.get("warned"):
                info["warned"] = True
                changed = True
                await premium_dm(uid, discord.Embed(
                    title="⏳ Ton premium expire bientôt",
                    description=(f"Il expire {premium_expiry_text(expires_at)}.\n"
                                 f"👉 [Renouveler]({PREMIUM_LINK})"),
                    color=C_GOLD))
...
def premium_status():
    ...
    expiry = pd["users"][did].get("expires_at", 0)

    now = int(time.time())

    if not expiry:

        return jsonify({"premium": True, "lifetime": True, "days_left": -1})

    return jsonify({"premium": expiry > now, "lifetime": False, "days_left": max(0, (expiry - now) // 86400)})
